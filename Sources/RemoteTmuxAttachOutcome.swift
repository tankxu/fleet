...
enum RemoteTmuxAttachOutcome: Sendable {
    /// The host's sessions were mirrored into `windowId`; `workspaceIds` are the
    /// mirrored session workspaces in that window.
    case mirrored(windowId: UUID, workspaceIds: [UUID])
